...
namespace ayr
{
	class TcpServer : public Object<TcpServer>
	{
		...
		void accept() { clients_.push_back(socket_.accept()); }
		...
		CString recv(int index, int flags) const { return client(index).recv(flags); }
	...
	class TcpClient : public Object<TcpClient>
	{
		...
		CString recv(int flags) const { return socket_.recv(flags); }
	private:
		Socket socket_;
	};
}
