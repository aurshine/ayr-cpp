...
void test_uri()
{
	...
	UTEST_EXPECT_EQ(full.scheme(), "https"as);
	UTEST_EXPECT_EQ(full.host(), "example.com"as);
	UTEST_EXPECT_EQ(full.port(), "8443"as);
	UTEST_EXPECT_EQ(full.path(), "/api/v1"as);
	UTEST_EXPECT_EQ(full.queries().get("q"as), "openai"as);
	UTEST_EXPECT_EQ(full.queries().get("page"as), "2"as);
	UTEST_EXPECT_EQ(full.fragment(), "top"as);

	net::Uri fragment_only = net::uri("https://example.com/path#frag"as);
	UTEST_EXPECT_EQ(fragment_only.fragment(), "frag"as);

	net::Uri relative = net::uri("/search?q=cpp#result"as);
	UTEST_EXPECT_EQ(relative.host(), ""as);
	UTEST_EXPECT_EQ(relative.path(), "/search"as);
	UTEST_EXPECT_EQ(relative.queries().get("q"as), "cpp"as);
	UTEST_EXPECT_EQ(relative.fragment(), "result"as);

	net::Uri bare_query = net::uri("https://example.com/path?flag=1&empty=2#frag"as);
	UTEST_EXPECT_EQ(bare_query.queries().get("flag"as), "1"as);
	UTEST_EXPECT_EQ(bare_query.queries().get("empty"as), "2"as);
}
...
void test_request()
{
	...
	UTEST_EXPECT_EQ(request.path(), "/search?q=cpp"as);
	UTEST_EXPECT_EQ(request.headers.get("host"as), "example.com"as);
	UTEST_EXPECT_EQ(request.headers.get("CONTENT-LENGTH"as), "6"as);
	UTEST_EXPECT_EQ(request.body, "你好"as.encode());
	...
	UTEST_EXPECT(!request.headers.contains("content-length"as));
	UTEST_EXPECT_AYR_ERROR(request.add_header("Bad Header"as, "value"as));
	UTEST_EXPECT_AYR_ERROR(request.add_header("X-Test"as, "one\r\ntwo"as));

	net::Uri manual_uri;
	manual_uri.scheme("HTTP"as);
	manual_uri.host("example.com"as);
	net::HttpRequest manual_request("GET"as, manual_uri, "HTTP/1.1"as);
	UTEST_EXPECT_EQ(manual_request.uri().scheme(), "http"as);
	tlog(manual_request.path());
	UTEST_EXPECT_EQ(manual_request.path(), "/"as);
}
...
void test_fixed_length_response()
{
	...
	UTEST_EXPECT_EQ(response.headers.get("Content-Length"as), "5"as);
...
void test_chunked_response()
{
	...
	UTEST_EXPECT_EQ(response.headers.get("x-trailer"as), "done"as);
