...
int main()
{
	UTEST_SCOPE("测试 then 构成的依赖图：子任务必须等父任务执行后才能运行。")
	{
		...
		auto task4 = exec.create_task([&] {
			UTEST_EXPECT(task2_done.load());
			UTEST_EXPECT(task3_done.load());
			task4_done = true;
			++completed;
		});
		auto task5 = exec.create_task([&] {
			UTEST_EXPECT(task4_done.load());
			++completed;
		});
		...
		task4->then(task5);
		exec.run();
		UTEST_EXPECT_EQ(completed.load(), 5);
	};
	
	UTEST_SCOPE("测试带返回值任务能把 future 结果传回调用方。")
	{
		async::AsyncExecutor ret_exec(2);
		auto [ret_task, future] = ret_exec.create_ret_task([] { return 40 + 2; });
		(void)ret_task;
		ret_exec.run();
		UTEST_EXPECT_EQ(future.get(), 42);
	};

	UTEST_SCOPE("测试 clear 会移除尚未运行的任务。")
	{
